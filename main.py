...
async def whenrating(ctx, name, rating, variant="Rapid"):
    error_bool, error_msg, prob_success, predicted_date, current_rating = await process_inputs(name,rating,variant)
    ...
    if error_bool and prob_success is None:
        ...
        if error_bool:
            msg += error_msg + '\n'
        msg += f"{name}'s chance of reaching {rating} {variant.title()} within 2 years: **{prob_success}%** (currently: {current_rating})"
        if prob_success > 5:
